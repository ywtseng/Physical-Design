...
bool BookshelfParser::readAuxFile(string filePathName)
{
    ...
    if (!auxFile) {
        cout << format("[Error] Cannot open aux file \"%s\"",
    ...
    if (slashPos != string::npos) {
        filePath = filePathName.substr(0, slashPos + 1);
    }
    ...
    if(_placement.plname()!=""){
        ...
        readPlFile(filePath + plFileName);

    return true;
}

bool BookshelfParser::readNodesFile(string filePathName)
{
    ifstream nodesFile(filePathName.c_str());
    if (!nodesFile) {
        cout << format("[Error] Cannot open nodes file \"%s\"\n",
                       filePathName.c_str()) << endl;
        return false;
    }
    ...
    while (getline(nodesFile, line)) {
        ...
        if (token == "NumNodes") {
            ...
            while (strStream >> token) {
                if (token == "terminal" || token == "terminal_NI") {
                    terminal = true;
    ...
    if (numModulesRead + numTerminalsRead != numNodes) {
        cout << "[Warning] numNodes in .nodes do not match the number of nodes read" << endl;
        cout << numModulesRead << " " << numTerminalsRead <<" " << numNodes << endl;
    }
    if (numTerminalsRead != numTerminals) {
        cout << "[Warning] numTerminals in .nodes do not match the number of terminals read" << endl;
    }
...
bool BookshelfParser::readNetsFile(string filePathName)
{
    ifstream netsFile(filePathName.c_str());
    if (!netsFile) {
        cout << format("[Error] Cannot open nets file \"%s\"\n",
                       filePathName.c_str()) << endl;
        return false;
    }
    ...
    if (numNetsRead != numNets) {
        cout << "[Warning] numNets in .nets do not match the number of nets read" << endl;

    }
    if (numPinsRead != numPins) {
        cout << "[Warning] numPins in .nets do not match the number of pins read" << endl;
    }
...
bool BookshelfParser::readSclFile(string filePathName)
{
    ifstream sclFile(filePathName.c_str());
    if (!sclFile) {
        cout << format("[Error] Cannot open scl file \"%s\"\n",
                       filePathName.c_str()) << endl;
        return false;
    }
    ...
    while (getline(sclFile, line)) {
        ...
        if (token == "Numrows" || token == "NumRows") {
            ...
            strStream >> token;
            if (token != "Horizontal") {
                cout << format("[Warning] CoreRow format \"%s\" in .scl is not supported",
                               token.c_str()) << endl;
            }
            ...
            if (token == "N") {
                ...
                siteOrient = Row::OR_FN;
            } else if (token == "FW") {
                siteOrient = Row::OR_FW;
            } else if (token == "FS") {
                siteOrient = Row::OR_FS;
            } else if (token == "FE") {
                siteOrient = Row::OR_FE;
            }
            ...
            if (++numSubRowRead > 1) {
                cout << "[Warning] number of sub-rows > 1 in .scl is not supported" << endl;
            }
    ...
    if (numRowsRead != numRows) {
        cout << "[Warning] numRows in .scl do not match the number of rows read" << endl;
    }
...
bool BookshelfParser::readPlFile(string filePathName)
{
    ...
    while (getline(plFile, line)) {
        ...
        if (token == "N") {
            moduleOrient = Module::OR_N;
        } else if (token == "W") {
            moduleOrient = Module::OR_W;
        } else if (token == "S") {
            moduleOrient = Module::OR_S;
        } else if (token == "E") {
            moduleOrient = Module::OR_E;
        } else if (token == "FN") {
            moduleOrient = Module::OR_FN;
        } else if (token == "FW") {
            moduleOrient = Module::OR_FW;
        } else if (token == "FS") {
            moduleOrient = Module::OR_FS;
        } else if (token == "FE") {
            moduleOrient = Module::OR_FE;
        }
